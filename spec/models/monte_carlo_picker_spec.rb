...
RSpec.describe MonteCarloPicker do
  let (:epics) {
    [
  ...
  describe "#play_once" do
    it "returns randomly selected values for the epic sizes given" do
      ...
      result = picker.play_once('S' => 2, 'M' => 3)
      expect(result).to eq({
  ...
  describe "#play" do
    it "plays the simulator 100 times and forecasts using the results" do
      ...
      forecast = picker.play(dummy_opts)
  ...
  def stub_rand_and_return(values)
    index = 0
    allow(picker.random).to receive(:rand).with(anything) do
      index += 1
      values[index - 1]
    end
  end
  ...
  def stub_simulator_to_play(results)
    index = 0
    allow(picker).to receive(:play_once) do
      index += 1
